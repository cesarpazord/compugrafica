...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Carga de modelo, lo vamos a dibujar por medio de pivotes (jerarquías)
        view = camera.GetViewMatrix();	
		model = glm::mat4(1);
...
void Animation() {
	...
	head = sin(glfwGetTime() * 3.0f) * 10.0f;
	tail = sin(glfwGetTime() * 6.0f) * 25.0f;

	// movimiento del perro
	dogPos.z += 0.005f;
	dogRot = sin(glfwGetTime()) * 5.0f;
}
